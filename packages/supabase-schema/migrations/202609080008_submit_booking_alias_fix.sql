-- Migration 008: fix ambiguous column reference in submit_booking.
-- The lock-attachment subquery aliased jsonb_array_elements(p_items) as
-- "v_item", colliding with the plpgsql loop variable of the same name.
-- Renames the subquery alias; no behavioral change otherwise.
