-- Migration 006: auth.users.confirmed_at is a GENERATED column (derived from
-- email_confirmed_at), so it must never be inserted or updated explicitly.
-- Supersedes the function bodies from migration 005 without that column and
-- completes the row repair that 005's final statement could not run.
...
-- Complete the repair that migration 005's final statement could not run.
update auth.users
