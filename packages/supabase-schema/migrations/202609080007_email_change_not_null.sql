-- Migration 007: auth.users.email_change must never be NULL.
-- GoTrue scans email_change into a non-nullable string; a NULL yields
-- "Database error querying schema" at login. Verified experimentally:
-- rows with email_change = '' log in, identical rows with NULL do not.
-- Adds the empty-string default to the account-creating functions and
-- repairs any rows created by migrations 004-006.
...
-- Repair rows created by earlier migrations that still carry a NULL email_change.
update auth.users
set email_change = ''
